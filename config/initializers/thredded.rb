...
Thredded.user_path = lambda { |user|
  ...
  main_app.respond_to?(user_path) ? main_app.send(user_path, user) : "/u/#{user.name}"
}
...
Thredded.email_from = ORG::EMAIL

# Emails going out will prefix the "Subject:" with the following string
Thredded.email_outgoing_prefix = "[#{ORG::NAME}] "
#
...
Thredded.messageboard_name_length_range = (3..60)

# The range of valid topic title lengths.
Thredded.topic_title_length_range = (3..150)
...
Thredded.slugifier = ->(input) { Babosa::Identifier.new(input).normalize.transliterate(:spanish).to_s }
...
# Lower to 1 if have 1-letter names -- increase if you want
Thredded.autocomplete_min_length = 3
